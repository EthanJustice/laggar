...
impl Url {
    pub fn new(original: String) -> Url {
        ...
        let url_path = String::from(Url::get_url_path(&parsed_url, &url_root));
    ...
    fn parse(mut url: String) -> String {
        // Parses user-provided input
        if url.starts_with("http") == false {
            // not a fully-qualified url, doesn't contain http/https
    ...
    fn get_url_path(url: &String, domain: &String) -> String {
        // formats url path to file system compatible version
        url.to_string()
            .replace("https://", "")
            .replace("http://", "")
            .replace("/", ".")
            .replace("..", ".")
            .replace(format!("{}.", domain).as_str(), "")
    }
}
...
mod tests {
    ...
    fn check_urls() {
        ...
        let domain_no_protocol = Url::new(String::from("example.com/about"));
        let domain_no_protocol_no_trail = Url::new(String::from("example.com"));
        ...
        assert_eq!(
            domain_no_protocol.parsed,
            String::from("http://example.com/about/")
        );
        assert_eq!(
            domain_no_protocol_no_trail.parsed,
            String::from("http://example.com/")
        );
        assert_eq!(
            sub_domain_no_trail.parsed,
            String::from("https://example.example.com/")
        );

        // roots
        assert_eq!(domain_no_trail.root, String::from("example.com"));
        assert_eq!(domain_no_protocol.root, String::from("example.com"));
        assert_eq!(
            domain_no_protocol_no_trail.root,
        ...
        assert_eq!(domain_no_protocol.url_path, String::from("about."));
        assert_eq!(domain_no_protocol_no_trail.url_path, String::from(""));
